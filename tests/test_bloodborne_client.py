...
class GrantCommandTests(unittest.TestCase):
    ...
    def test_protocol_and_harness_versions_move_together(self):
        self.assertEqual(bb_client.RUNTIME_BUILD, "bb-0.1.0-r10")
        self.assertEqual(bb_client.BRIDGE_PROTOCOL, "BBGRANT1")
        self.assertEqual(bb_client.HARNESS_VERSION, "bb-native-grant-v7")
...
class ManualCheckTests(unittest.TestCase):
    def test_a_check_is_appended_to_the_journal(self):
        ...
        location = LOCATION_ID_BY_KEY["boss_father_gascoigne"]
        ...
        self.assertEqual(records[0]["location_name"], "Father Gascoigne")
    ...
    def test_a_typo_gets_a_nearby_location_name(self):
        suggestions = bb_client.location_suggestions("Father Gascoyne")
        self.assertIn("Father Gascoigne", suggestions)
