...
| The client sits at `1 queued` and nothing arrives; a `?GoodsName?` item is in the Hunter's Dream storage box | An undelivered rune or gem token went to storage. Withdraw it, one at a time, and the item arrives. See KNOWN-LIMITATIONS. |
