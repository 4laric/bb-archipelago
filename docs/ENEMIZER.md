...
Every launcher build keeps the exact plan the writer consumed in the seed
cache, beside the seed manifest and outside the overlay file set
(`bb-enemizer-plan.json`, hash-recorded in the manifest's `enemizer.plan`
record and verified with the rest of the build). Each planned swap carries
its destination coordinates and entity id, the size/tier/locomotion tags of
both archetypes, and the NpcParam facts from
`research/enemizer/archetype_facts.json` (designer row name, Blood Echo
reward, HP; regenerated by `tools/build_enemizer_facts.py` from the bundle).
A bad transplant is therefore nameable after the fact without the game dump,
which bb-archipelago#321 showed was the missing piece.

The player-facing surface is **Report a Bad Enemy** in the launcher's
recovery actions, or the same thing from a checkout:

```powershell
python -m bb_launcher enemy-report --settings launcher-settings.json `
  --area m24_01 --echoes 4700 --note "something keeps dying by the lamp"
```

Both write one Markdown file under the launcher state root
(`enemy-reports/`), copy it to the clipboard, and open it. The report names
the seed, slot, cache key, plan hash, enemy seed, options, and any stress
profile, then lists every swap in the chosen area in plain words. When the
player saw an echo reward, the enemies whose reward is nearest come first,
...
| --- | --- |
| `stress:size-up:m24_01` | takes the largest size step up the rule allows (`+1` today) |
| `stress:size-down:m24_01` | takes the largest size step down the rule allows |
| `stress:locomotion:m24_01` | changes locomotion class wherever a candidate exists |
| `stress:tier-up:m24_01` | takes the highest-tier candidate; implies tier mixing |
| `stress:echoes:m24_01` | takes the candidate with the largest Blood Echo reward (#321's shape) |
| `stress:family=c4060:m24_01` | takes the named model family wherever it is compatible |

The map prefix keeps every other map vanilla so the tester knows where to
look; omit it to stress the whole game. The profile only reorders which
compatible candidate wins: the protection policy, the size and tier gates,
and the writer's invariants are untouched, and a family that is never
compatible in the focus simply matches nothing (the plan's `stress.matched`
count says so). Profiles are deterministic like any seed, so a report made
from one names the same swap on every machine. Seeds without the `stress:`
prefix are planned exactly as before; the 308-swap pin is unchanged.
