...
def native_plan_laurence_at_cleric(slots,npcs,effects,ids,seed):
 ...
 a,d=cleric[0],donor[0];sw=Swap(a.logical_key,[x.key for x in cleric],{x.key:x.archetype for x in cleric},a.archetype,d.archetype,destinations={x.key:{'map_name':x.map_name,'entity_id':x.entity_id,'x':x.x,'y':x.y,'z':x.z} for x in cleric});c,k=plan_scaling([sw],cleric,dict(npcs),dict(effects))
