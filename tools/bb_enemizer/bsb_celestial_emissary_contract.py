"""Pinned Blood-starved Beast combat in the Celestial Emissary arena.

The visible c2500 body is BSB.  The native Celestial terminal retains its c2570
subject and an added bridge kills that giant only after BSB dies.  Runtime
...
TERMINAL = 12421700
PROJECT_MIN, PROJECT_MAX = 12992300, 12992399
...
class BsbCelestialIds:
    phase_one: int = 12992300
    phase_two: int = 12992301
    death_to_giant: int = 12992302
    retire_celestial: int = 12992303
...
def _replace_once(text, old, new, label):
    if text.count(old) != 1:
        raise ValueError(f"BSB/Celestial expected one {label}")
...
def _validate_ids(ids, destination):
    ...
    if (
        ...
        raise ValueError(
            "BSB/Celestial IDs must be collision-free project-owned 129923xx values"
        )


def _initializer(event_zero, source_event, dest_event):
    calls = [
        x
...
def _health(donor, ids):
    ...
    return _replace_once(
        result, "CreatePlaylog(86);", "CreatePlaylog(104);", "Celestial playlog"
    )
...
def _camera(donor):
    ...
    result = _replace_once(
        ...
        "Celestial camera binding",
    )
    if "SetLockcamSlotNumber(23," in result:
        raise ValueError("BSB/Celestial copied BSB camera map")
    return result
...
def patch_bsb_at_celestial_emissary(destination, donor_source, ids=DEFAULT_IDS):
    ...
    edits = {
        0: constructor,
        12421702: _remap(donor[12301802], mapping),
        12421703: _remap(donor[12301803], mapping),
        12424702: _health(donor[12304802], ids),
        12424703: _remap(donor[12304803], mapping),
        12424704: _camera(donor[12304804]),
    ...
    if set(output) != set(arena).union(ids.values()):
        raise ValueError("BSB/Celestial changed event identities")
    for x, b in arena.items():
        if x not in edits and output[x] != b:
            raise ValueError(f"BSB/Celestial changed unrelated arena event {x}")
    if output[TERMINAL] != arena[TERMINAL]:
        raise ValueError("BSB/Celestial changed destination giant terminal")
    for x in EBRIETAS_EVENTS:
        if output[x] != arena[x]:
            raise ValueError(f"BSB/Celestial changed shared Ebrietas event {x}")
    copied = "\n".join(
        output[x]
        for x in (12421702, 12421703, 12424702, 12424703, 12424704, *ids.values())
    )
    if re.search(r"(?<!\d)(?:123|230)\d+(?!\d)", copied):
        raise ValueError("BSB/Celestial copied combat retains donor-map literals")
    return result
...
def _require(slots, entity, archetype, map_name=None):
    ...
    if not found or any(x.dummy or x.archetype != archetype for x in found):
        raise ValueError(f"unsupported BSB/Celestial placement provenance for {entity}")
    return found
...
def native_plan_bsb_at_celestial_emissary(
    slots: Sequence[Slot],
    npcs: Mapping[int, dict],
    effects: Mapping[int, dict],
    seed: str,
    ids: BsbCelestialIds = DEFAULT_IDS,
):
    ...
    if (
        {x.map_name for x in bsb} != set(BSB_SOURCE_PINS)
        or len(bsb) != 2
        or len(primary) != 1
        or len(giant) != 1
        or len(helpers) != 9
        ...
        raise ValueError(
            "BSB/Celestial requires two BSB states, a primary, giant, and nine retained helpers"
        )
    source = next(x for x in bsb if x.map_name == "m23_00_00_00")
    target = primary[0]
    swap = Swap(
        target.logical_key,
        [target.key],
        ...
        BSB_ARCHETYPE,
        warnings=[
            "experimental BSB-at-Celestial contract; runtime behavior unobserved"
        ],
        destinations={
            target.key: {
    ...
    if len(changes) > 1 or (changes and skips):
        raise ValueError(
            "BSB/Celestial primary swap has an ambiguous normalization plan"
        )
    retained = []
        ...
        retained.append(
            {
                ...
                "policy": (
                    "retain_giant_disabled_alive_until_bsb_death_bridge"
    ...
    return {
        ...
        "swaps": [swap.json()],
        "boss_contract": {
            "format": "bb-bsb-celestial-emissary-contract-v1",
            ...
            "terminal_policy": "retain byte-identical giant terminal; bridge BSB death to giant only afterward",
