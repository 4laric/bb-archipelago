...
if (args.Length == 3 && args[0] == "--event-fingerprint")
    return WakeupFallback.PrintFingerprint(args[1], long.Parse(args[2], System.Globalization.CultureInfo.InvariantCulture));
...
if (args.Length == 2 && args[0] == "--boss-actor-pins")
...
if (args.Length == 12 && args[0] == "--boss-encounters" && args[9] == "--characters" && args[11] == "--apply")
    return BossEncounter.Run(args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8], charactersPath: args[10]);
...
if (args.Length != 4 || args[3] != "--apply")
{
    ...
    Console.Error.WriteLine(
        "Boss encounters: BBEnemizerWriter --boss-encounters <plan> <built-gameparam> <paramdef> <maps> <scripts> <event-input-dir> <compiled-event-dir> <new-output-root> [--sfx <original-sfx-dir>] [--characters <original-character-dir>] --apply");
